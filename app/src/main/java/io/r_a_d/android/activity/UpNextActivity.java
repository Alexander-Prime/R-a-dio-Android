...
implements   RadioInfoService.Listener,
	...
	@Override protected void onCreate( Bundle state ) {
		...
		radioInfoService_connection = new ServiceConnection() {
			@Override public void onServiceConnected( ComponentName name, IBinder binder ) {
				...
				radioInfo_binder.addListener( UpNextActivity.this );
			...
			@Override public void onServiceDisconnected( ComponentName name ) {
				radioInfo_binder.removeListener( UpNextActivity.this );
			}
		};
		...
		radioPlaybackService_connection = new ServiceConnection() {
			@Override public void onServiceConnected( ComponentName name, IBinder service ) {
				...
				radioPlayback_binder.addListener( UpNextActivity.this );
				onPlaybackStateChanged( radioPlayback_binder.getPlaybackState() );
				playPause_drawable.endAnimation();
			}

			@Override public void onServiceDisconnected( ComponentName name ) {
				radioPlayback_binder.removeListener( UpNextActivity.this );
			}
		};
	}
	...
	private class Adapter extends RecyclerView.Adapter<ViewHolder> {
		...
		@Override public void onBindViewHolder( ViewHolder holder, int position ) {
			...
			final CharSequence relativeTime = DateUtils.getRelativeTimeSpanString( songs.get( position ).timestamp * 1000, now, 0 );

			holder.time.setText( relativeTime );

			if ( Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP ) {
				if ( position == 0 ) holder.title.setTransitionName( "upNext_title" );
